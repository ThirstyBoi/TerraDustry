	...
	draw: function(b){
		...
		for(var s = 0; s < 4; s++){
			...
			for(var i = 0; i < 4; i++){
				...
				Lines.stroke((4 + Mathf.absin(Time.time() + (10 * s), 1.9, 1.8)) * b.fout() * strokes[s] * tscales[i]);
...
const rainbow = extendContent(LaserTurret, "beelzebub",{
	load(){
		...
		this.baseRegion = Core.atlas.find("overdriven-block-" + this.size);
	},
	
	generateIcons: function(){
	return [
		Core.atlas.find("overdriven-block-" + this.size),
